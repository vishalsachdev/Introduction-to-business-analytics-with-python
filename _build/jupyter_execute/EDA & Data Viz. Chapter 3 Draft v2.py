...
# ```
# 
# The sample function randomly selects n rows to display, where n is a specified argument
# 
# ```
# random_df.sample(5)
# ```
...
# **Data Visualization:**
# 
# Data visualization calls for visualizing relationships between variables. In this chapter, we will review some common data visualization methods.
...
# 
# ```
# ax = random_df.groupby('categorical_variable_of_interest').categorical_variable_of_interest.mean().plot(kind='bar', figsize=[10, 6], color='orangered')
...
# ```
# 
# Next, set Date column as index. This is a time series, not a data frame.
# ```
...
# 
# We can generate a bar chart to quickly see which variables are missing values. 
# ```
...
# **Pivot Tables:**
# 
# A pivot table is used to summarize a large data set by grouping data together via multiple columns. This can be done using the pivot_table function, first taking one argument which will be the aggregated column. The index column then takes the row indices, specifying how the table will be grouped and the column arguments specifies the columns that are displayed in the table. Finally, the aggfunc argument specifies what functions should be computed and displayed, with average being the default.
# 
# ```
# random_df.pivot_table(index='var1', columns='var2', aggfunc={'totall': ['max', 'median', 'min']})
# ```
