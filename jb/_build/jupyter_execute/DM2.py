...
# 
# To help determine which model generalizes to all data better, we have added in some new data points (pink) of which we do not know the type (red or blue).
# 
...
# 
# With the black line, the complexity is just right. Though the model is simpler, it still captures key information from the data. While it might misclassify a few of the data points that we’re learning from, it is more likely to accurately classify new data points.
# 
# Overfitting could occur for many reasons, such as the inclusion of superfluous or irrelevant predictors. If the number of predictors in the model approaches the number of records in the dataset, we could start picking up spurious relationships. Using a high order polynomial, like the green model in the previous example, is often less favorable to using a simpler model, like the black model in the previous example.
# 
# Overfitting could also be a result of trying too many different models which may lead you to relationships that do not exist by chance. This means that if you agonize over the data for too long, patterns may emerge whether or not they are generalizable to new data.
...
# ![image.png](attachment:image.png)
# 
# This will be the data in my validation or test set. For each record we have an actual outcome value.
...
# ![image.png](attachment:image.png)
# 
# The actual outcome will be known as _y _and the predicted outcome will be known as _y-hat._
...
# 
# Both the root mean squared error and mean absolute error give us a sense of the absolute error, and both are used to assess performance of _predictive_ models.
# 
